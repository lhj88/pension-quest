...
관리자는 `/admin/settings`의 `참가자 초기화` 구역에서 테스트용 참가자 기록을 지울 수 있습니다.
...
초기화가 완료되면 게임 상태는 `진행중`으로 되돌아갑니다. 실수 방지를 위해 확인 문구 입력칸에 정확히 `초기화`라고 입력해야 실행됩니다.
...
5. 테스트 기록을 지우거나 새 게임을 시작하려면 `/admin/settings`에서 참가자 초기화를 실행합니다.
6. 행사가 끝나면 `/admin/settings`에서 `잠금`으로 바꾸거나 `/admin/draw`에서 바로 추첨을 실행합니다.
7. `/results`를 휴대폰 또는 TV에 띄워 결과와 리더보드를 보여줍니다.
